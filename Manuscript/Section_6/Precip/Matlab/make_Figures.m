% In this file we show the obsdrved and predicted precipitation as well as the scores
% for the various precipitation models
%mesh
loc = load('delLoc.txt');
tv = load('delTV.txt');
...
set(gca, 'FontSize', 28);
xlabel('Easting (km)', 'FontSize', 28)
ylabel('Northing (km)', 'FontSize', 28)
colorbar;
clim([0.35, 3.7])
view(0, 90);
hold on;
...
hmm = figure;
set(hmm, 'Position', [100, 100, 625, 600]);
...
hmm = figure;
set(hmm, 'Position', [100, 100, 625, 600]);
scatter(oLoc(:,1), oLoc(:,2), 80, obs, 'filled');
hold on;
plot(border(:,1), border(:,2), 'k');
xlim([-150, 550]);
ylim([6400, 7300]);
...
function create_point_plot(oLoc,  border,  data, file_name)
    hmm = figure;
    set(hmm, 'Position', [100, 100, 650, 610]);
    ...
    scatter(oLoc(:,1), oLoc(:,2), 80, data, 'filled');
    ...
    outerpos = ax.OuterPosition;
    ti = ax.TightInset;
    buffer = 0.05; % Adjust buffer size as needed
    left = outerpos(1) + ti(1) + buffer; % Increase left buffer
    bottom = outerpos(2) + ti(2) + buffer; % Increase bottom buffer
    ax_width = outerpos(3) - ti(1) - ti(3) - buffer * 2; % Reduce width to add buffer on both sides
    ax_height = outerpos(4) - ti(2) - ti(4) - buffer * 2; % Reduce height to add buffer on top and bottom
    ax.Position = [left bottom ax_width ax_height];
