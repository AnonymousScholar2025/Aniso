# DESCRIPTION: This file includes functions for working with the PC prior, including calculation of the prior, posterior given a
# noisy observation, the MAP estimate, importance sampling and credible intervals.
...
fdist_prime <- function(r) {
  sqrt(pi) * sinh(2 * r) / (sqrt(cosh(2 * r) + 1 / 3))
}
...
PC_prior_log_kappa <- function(log_kappa, lambda, lambda1) {
  ...
  fact <- (exp(-2 * sqrt(pi / 3) * kappa * lambda) * lambda * lambda1 * (3 + 2 * sqrt(3 * pi) * (kappa * lambda + lambda1))) / (3 * (kappa * lambda + lambda1)^2)
  return(change_variable * fact)
}
#' @title Marginal PC prior density of kappa
#' @description Calculates  the marginal PC prior pi_kappa of the inverse correlation range kappa
#' @param lambda A hyperparameter controlling the penalization of the distance from the base model as a function of kappa
#' @param lambda1 A hyperparameter controlling the penalization of the distance from the base model as a function of v
#'
#' @return The marginal prior density function of kappa .

PC_prior_kappa <- function(lambda, lambda1) {
  pc_kappa <- function(kappa) {
    log_kappa <- log(kappa)
    change_variable <- 1 / kappa
    PC_prior_log_kappa(log_kappa, lambda, lambda1) * change_variable
  }
  return(pc_kappa)
}
#' @title Marginal PC prior density function of kappa
#' @description Calculates  the marginal PC prior density pi_kappa
#'
#' @param rho0 Surprisingly small correlation range
#' @param a0 Surprisingly large amount of anisotropy
#' @param alpha Quantile by default 0.01
#'
#' @return The marginal prior density function of kappa .
#' @export
PC_prior_kappa_quantile <- function(rho0, a0, alpha = 0.01) {
  lambda1 <- lambda1_quantile(a0 = a0, alpha = alpha)
  lambda <- lambda_quantile(rho0 = rho0, lambda1 = lambda1, alpha)
  # Change of variable
  pc_kappa <- function(kappa) {
    log_kappa <- log(kappa)
    change_variable <- 1 / kappa
...
#' @title Prior density function for rho, anisotropic field
#' @description
...
pc_prior_rho <- function(rho0, a0, alpha = 0.01) {
  pc_rho <- function(rho) {
    # Change of variables
    kappa <- sqrt(8) / rho
    change_variable <- sqrt(8) / rho^2
    # PC on rho
    pc_kappa <- PC_prior_kappa_quantile(rho0, a0, alpha)
    pc_kappa(kappa) * change_variable
  }
  return(pc_rho)
}

#' @title Marginal PC prior density of log(r) = log(|v|)
#' @description Calculates  the marginal PC prior, pi_log(r), of the log norm of the anisotropy vector
#'
...
PC_prior_log_r <- function(log_r, lambda1) {
  r <- exp(log_r)
  change_variable <- r
  lambda1 * exp(-lambda1 * (fdist(r) - fdist(0))) * change_variable * fdist_prime(r)
}

#' @title Marginal PC prior density of r = |v|
#' @description Calculates  the marginal PC prior, pi_r, of the log norm of the anisotropy vector
#'
#' @param lambda1 A hyperparameter controlling the penalization of the distance from the base model as a function of r.
#' @param log_r The logarithm of |v|, which controls the magnitude of the anisotropy
#'
...
#' lambda1 <- 1
#' result <- PC_prior_r(log_r, lambda1)
PC_prior_r <- function(r, lambda1) {
  lambda1 * exp(-lambda1 * (fdist(r) - fdist(0))) * fdist_prime(r)
}
...
#' @param v A two dimensional vector that controls the direction and magnitude of anisotropy
#'
#' @return The marginal prior density function of v.
#' @export
#' @examples
#' v <- c(1, 2)
#' lambda1 <- 1
#' result <- PC_prior_v(v, lambda1)
PC_prior_v <- function(lambda1) {
  pc_v <- function(v) {
    norm_v <- sqrt(sum(v^2))
    PC_prior_r(norm_v, lambda1) / (2 * pi * norm_v)
  }
}
...
pc_prior_aniso <- function(lambda, lambda1, log_kappa, v) {
  ...
  term1 <- (lambda * lambda1 * f_prime_val * f_val) / (2 * pi * v_norm)
...
#' @param v 2D vector that controls anisotropy
#' @param log_sigma_u Variance of field u. If unspecified, it is assumed to be 0.
#' @param y A vector with length m equal to the number of columns of A representing the observed data.
...
#' @description Calculates the interval score for a given parameter and credible intervals
#' @param parameter A vector of length d representing the parameter
#' @param credible_intervals A matrix of size d x 2 representing the credible intervals
#' @param alpha The significance level. By default, set to 0.05.
#' @return A vector of length d representing the interval score for each dimension
#' @export

interval_score <- function(parameter, credible_intervals, alpha = 0.05) {
  # Calculate the interval score for each dimension
  parameter <- unlist(parameter)
  interval_score <- credible_intervals[, 2] - credible_intervals[, 1] + 2 / alpha * (credible_intervals[, 1] - parameter) * (parameter < credible_intervals[, 1]) + 2 / alpha * (parameter - credible_intervals[, 2]) * (parameter > credible_intervals[, 2])
...
convert_to_range_and_non_log <- function(theta) {
  ...
  if (dim(theta)[2] == 5) {
    theta[, 1] <- correlation_range(theta[, 1])
    theta[, 4:5] <- exp(theta[, 4:5])

    colnames(theta) <- c("rho", "v1", "v2", "sigma_u", "sigma_epsilon")
    ...
    if (dim(theta)[2] == 3) {
      ...
      colnames(theta) <- c("rho", "sigma_u", "sigma_epsilon")
      return(theta)
    } else {
      stop("The number of columns of theta must be 3 or 5")
    }
  }
}
#' @title Parameters to non log parameters
#' @description Converts parameters to non log parameters
#' @param theta A vector of parameters
#' @return A vector of non log parameters
#' @export
#' @examples
#' theta <- c(-1, 0.1, 0.1, -3, 1)
#' parameters_to_non_log(theta)
parameters_to_non_log <- function(theta) {
  theta <- unname(unlist(theta))
  t(list(rho = correlation_range(theta[1]), v1 = theta[2], v2 = theta[3], sigma_u = exp(theta[4]), sigma_epsilon = exp(theta[5])))
}

#' Half angle vector
#' @description Converts a vector to its half angle vector
#' @param v A 2D vector
#' @return The 2D half angle vector
#' @export

vector_to_half_angle_vector <- function(v) {
  v1 <- v[1]
  v2 <- v[2]
  if (v[1] == 0 && v[2] == 0) {
    return(v)
  } else {
    theta <- (atan2(v2, v1) %% (2 * pi)) / 2
    sqrt(v1^2 + v2^2) * c(cos(theta), sin(theta))
  }
}
